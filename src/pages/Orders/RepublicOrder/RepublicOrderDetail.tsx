...
const RegionOrderDetail: React.FC = () => {
  ...
  useEffect(() => {
    const handleWebSocketMessage = async (msg: string) => {
      try {
        ...
        if (parsed.certificates) {
          ...
          if (currentSigningType === 'buyurtma') {
            console.log("Buyurtma hati imzolandi, serverga yuborilmoqda...");
            
            try {
              const response = await axiosAPI.post("signing/upload", {
                document_name: "ЗаявкаПоРеспублика",
                number: signingData.number,
                id: id ?? "",
                data: parsed
              });
              if (response.status === 200) {
              ...
              toast.error(error.response?.data?.error || "Hujjatni imzolashda xatolik!");
            }
            
            // 🔴 IMZOLASH TUGAGANDA TOZALASH
            setEImzoOpen(false);
            setCurrentSigningType(null);
            setKeyID(""); // ← KeyID ni tozalash
            setSelectedCertificate(null); // ← Sertifikatni tozalash
            setSelectedCertRow(null); // ← Tanlangan sertifikatni tozalash
            
          } else if (currentSigningType === 'xizmat') {
            console.log("Xizmat hati imzolandi, serverga yuborilmoqda...");
            
            try {
              const response = await axiosAPI.post("signing/upload", {
                document_name: "ЗаявкаПоРеспублика", 
                number: xizmatSigningData.number,
                id: id ?? "",
                data: parsed
              });
              if (response.status === 200) {
              ...
              toast.error(error.response?.data?.error || "Xizmat hatini imzolashda xatolik!");
            }
    ...
    webSocketService.sendMessage(JSON.stringify({ 
      'name': 'apikey', 
      'arguments': ['null', 'E0A205EC4E7B78BBB56AFF83A733A1BB9FD39D562E67978CC5E7D73B0951DB1954595A20672A63332535E13CC6EC1E1FC8857BB09E0855D7E76E411B6FA16E9D', 'localhost', '96D0C1491615C82B9A54D9989779DF825B690748224C2B04F500F370D51827CE2644D8D4A82C18184D73AB8530BB8ED537269603F61DB0D03D2104ABF789970B', '127.0.0.1', 'A7BCFA5D490B351BE0754130DF03A068F855DB4333D43921125B9CF2670EF6A40370C646B90401955E1F7BC9CDBF59CE0B2C5467D820BE189C845D0B79CFC96F', 'test.e-imzo.uz', 'DE783306B4717AFE4AE1B185E1D967C265AA397A35D8955C7D7E38A36F02798AA62FBABE2ABA15C888FE2F057474F35A5FC783D23005E4347A3E34D6C1DDBAE5', 'test.e-imzo.local', 'D56ABC7F43A23466D9ADB1A2335E7430FCE0F46B0EC99B578D554333245FC071357AE9E7E2F75F96B73AEEE7E0D61AE84E414F5CD795D8B6484E5645CAF958FC'] 
    }));
  }, [currentSigningType, signingData.number, xizmatSigningData.number, id]);

  // Imzolash useEffect ni yangilang
  useEffect(() => {
    // Faqat modal ochiq va currentSigningType mavjud bo'lganda ishlaydi
    if (keyID && currentSigningType && 
        ((currentSigningType === 'buyurtma' && eImzoOpen) || 
        (currentSigningType === 'xizmat' && xizmatEImzoOpen))) {
      
      if (currentSigningType === 'buyurtma' && messageFileBinary) {
      ...
      } else if (currentSigningType === 'xizmat' && xizmatFileBinary) {
  ...
  const handleCloseXizmatEImzoModal = () => {
    setXizmatEImzoOpen(false);
    setCurrentSigningType(null);
    setKeyID("");
    setSelectedCertificate(null);
  ...
  return (
    ...
    {showIjrochiModal && (
      ...
      <Modal
        ...
        open={eImzoOpen}
        onCancel={handleCloseEImzoModal}
        style={{ minWidth: "800px" }}
        footer={[
          <Button key="cancel" onClick={handleCloseEImzoModal}>
            Chiqish
          </Button>,
          <Button
            key="ok"
      ...
      <Modal
        ...
        onCancel={handleCloseXizmatEImzoModal}
        style={{ minWidth: "800px" }}
        footer={[
          <Button key="cancel" onClick={handleCloseXizmatEImzoModal}>  
            Chiqish
          </Button>,
          <Button
            key="ok"
