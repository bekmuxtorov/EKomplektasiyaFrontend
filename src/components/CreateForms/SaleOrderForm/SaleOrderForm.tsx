...
const CREATE_ENDPOINT = "/sale-orders/create/";
...
const OrderWIndow: React.FC<IDistrictOrderFormProps> = ({ setIsCreateFormModalOpen }) => {
  ...
  const handleView = async (f: FileData) => {
    try {
      ...
      const res = await axiosAPI.get(`sale-orders/${docId}/file/${f.raw_number}`, {
        responseType: "blob",
      });
  ...
  const getDistrictOrderFile = async (id: string) => {
    if (id) {
      try {
        const response = await axiosAPI.get(`sale-orders/${id}/order-file`);
    ...
    try {
      const response = await axiosAPI.post(`/sale-orders/update/${documentID}`, payload);
      if (response.status === 200) {
